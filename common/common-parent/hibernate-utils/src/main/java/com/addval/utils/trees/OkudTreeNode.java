...
import com.addval.utils.LinkedListNode;
...
public class OkudTreeNode extends AOkudTreeNode
{
     ...
     */
    public OkudTreeNode(OkudTreeNode parent, Object key, Object childMap, boolean childDiscardable, boolean lruCacheUsed)
    {
        super(parent, key);
        _childDiscardable = childDiscardable;
        _lruCacheUsed = lruCacheUsed;
        if(_childDiscardable)_childMap = new SoftReference(childMap);
        else _childMap = childMap;
        
        _terminal = false;
        
        if(parent == null)_levelIndex = 0;
        else _levelIndex = parent.getLevelIndex() + 1;
    }
    ...
    public OkudTreeNode(OkudTreeNode parent, Object key, Object childMap, OkudTree tree, int levelIndex)
    {
        ...
        if(tree.isUseExternalCache() && _level.isUseExternalCache()){   
            ...
            if(this.getChildMap() == null)this.addToExternalCache(childMap);
        }
    ...
    public final Map getChildMap()
    {
        ...
        if(_useExternalCache){
            resultMap = (Map<Object, AOkudTreeNode>)getTree().getExternalCacheUtility().get(getTree().getExternalCacheName(), this.getExternalCacheKey());
        }
        else if(_childDiscardable){
            Object obj = ((SoftReference)_childMap).get();
            if(obj == null)return null;
            if(_lruCacheUsed){                
                resultMap = (Map<Object, AOkudTreeNode>)((OkudTree.LruDataObj)((LinkedListNode)obj).getValue()).getData();
            }
            else{
    ...
    {
        if(childMap == null)_childMap = null;
        else if(_useExternalCache)this.addToExternalCache(childMap);
        else if(_childDiscardable)_childMap = new SoftReference(childMap);
        else _childMap = childMap;        
    }
    ...
    public Object remove(Object key)
    {
        ...
        Object ret =  null;
        if(contains)ret = cmap.remove(key);

        //have to put it back in external cache
        if(contains)this.addToExternalCache(cmap);
    ...
    public SortedMap subMap(Object fromKey, Object toKey) 
    {
        Map cmap = getChildMap();
        if(cmap == null)return null;        
        if(!(cmap instanceof SortedMap))throw new UnsupportedOperationException("subMap(fromKey, toKey)");
    ...
    public SortedMap headMap(Object toKey)
    {
        Map cmap = getChildMap();
        if(cmap == null)return null;        
        if(!(cmap instanceof SortedMap))throw new UnsupportedOperationException("headMap(toKey)");
    ...
    public SortedMap tailMap(Object fromKey)
    {
        Map cmap = getChildMap();
        if(cmap == null)return null;        
        if(!(cmap instanceof SortedMap))throw new UnsupportedOperationException("tailMap(fromKey)");
    ...
    {   
    	...
    	try {
            ...
            if (lockObtained) {	
            	...
            	throw new AFLSCacheException("In Okud Tree", "Timed out while waiting for clear leaf node. Please Retry request.");
            }
        }
    ...
    final void addToExternalCache(Object cmap)
    {
        try{
            if(_useExternalCache){
                this.getCacheLock();
                this.getTree().getExternalCacheUtility().put(this.getTree().getExternalCacheName(), this.getExternalCacheKey(), cmap);            
            } 
        }
        finally{
    ...
    void getCacheLock()
    {
        //blocking
        if(_useExternalCache){
            this.getTree().getExternalCacheUtility().getWriteLock(this.getTree().getExternalCacheName(), this.getExternalCacheKey());
        }
    }
    ...
    {
        if(_useExternalCache){
            this.getTree().getExternalCacheUtility().releaseWriteLock(this.getTree().getExternalCacheName(), this.getExternalCacheKey());
        }        
    }
