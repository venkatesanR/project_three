...
/**
 * Implementation of EJBSTableManagerBean to work with EJB and spring. A session
 * bean implementation may be there to enable CRUD functions on a table/view that is
 * represented by EditorMetaData. The bean/ utility returns EJBResultSet objects for
 * lookup and update functions
 */
public class AbstractEJBSTableManager implements EJBSTableManager 
{
	private static transient final Logger _logger = com.addval.utils.LogMgr.getLogger(AbstractEJBSTableManager.class);

	private Environment _env = null;
	private Map<String,Environment> envInstances = null;
	private static final transient String _module = "com.addval.ejbutils.server.EJBSTableManagerBean";
	...
	private Hashtable<String, TableManagerInterceptor> _interceptorMap = null;
	 ...
	 */
	public AbstractEJBSTableManager() {
	...
	public void setCallerPrincipalName(String aName) throws RemoteException {
	...
	public void setInterceptorMap(Hashtable<String, TableManagerInterceptor> aMap) {
		_interceptorMap = aMap;
	}

	public Hashtable<String, TableManagerInterceptor> getInterceptorMap() {
	...
	/**
	 * Given a criteria, this method will retrieve the data into an EJBResultset
	 * (performs a 'select') Can also be used to lock a table using the 'for
	 * update of' clause
	 *
	...
	public EJBResultSet lookup(EJBCriteria criteria, boolean lock) {
		...
		try {
			...
			if (lock) {
				...
				Vector<ColumnMetaData> keyColumns = metaData.getEditorMetaData().getKeyColumns();
				if (keyColumns != null) {
					Iterator<ColumnMetaData> iterator = keyColumns.iterator();
					if (iterator.hasNext()) {
	...
	/**
	 * Lookup the database and return a resultset based on the EJBCriteria that
	 * is provided. Forces the record to be locked
	 *
	 * @param criteria
	...
	/**
	 * Lookup the database and return a resultset based on the EJBCriteria that
	 * is provided. This will get a read-only copy - no lock is placed on the
	 * table/row
	 *
	 * @param criteria
	 * @return com.addval.ejbutils.dbutils.EJBResultSet
	...
	public EJBResultSet lookup(String sql, EditorMetaData editorMetaData, EJBCriteria criteria) {
		...
		try {
			...
			Vector<ColumnMetaData> columnList = new Vector<ColumnMetaData>(sqlColCount);
	 ...
	 * (insert/update/delete). Records that fail updates have their status set
	 * to _RSS_ERROR
	 *
	 * @param rs
	 * @return com.addval.ejbutils.dbutils.EJBResultSet
	...
	/**
	 * Similar to the update method, but if one of the records fail to update,
	 * the method will throw an exception thus rolling back the full
	 * transaction.
	 *
	 * @param rs
	 * @return com.addval.ejbutils.dbutils.EJBResultSet
	...
	public EJBResultSet updateTransaction(EJBResultSet rs) throws EJBXRuntime {
		...
		try {
			...
			// rollback happens
			if (errorMessage != null) {
				doTransactionRollback( se, errorMessage );
	...
	protected void doTransactionRollback(Exception exception, String errorMessage) 
	{
	...
	/**
	 * Method for updating the Master/Detail Screens. Will do a) MasterTable
	 * Insert, b) detail table delete and then c) detail table inserts. All the
	 * above operations are done as a single transaction.
	 *
	 * @param masterSet
	 * @param deleteSet
	 * @param insertSet
	...
	/**
	 * Internal method to perform operations on a given connection. Does not
	 * perform any transaction control
	 *
	 * @param rs
	 * @param conn
	 * @return com.addval.ejbutils.dbutils.EJBResultSet
	...
	private int insert(EJBResultSet rs, Connection conn) throws SQLException
	{
		...
		if (rs.getEJBResultSetMetaData().getEditorMetaData().getProcedure() != null && rs.getEJBResultSetMetaData().getEditorMetaData().getProcedure().startsWith("CALL"))
			...
			rv = execUpdate(sqlBuilder, sqlBuilder.buildInsertSql(rs, getCallerPrincipalName(),conn), rs, conn);
	...
	private int update(EJBResultSet rs, Connection conn) throws SQLException
	{
		...
		if (rs.getEJBResultSetMetaData().getEditorMetaData().getProcedure() != null && rs.getEJBResultSetMetaData().getEditorMetaData().getProcedure().startsWith("CALL"))
			...
			rv = execUpdate(sqlBuilder, sqlBuilder.buildUpdateSql(rs, getCallerPrincipalName()), rs, conn);
	...
	private int delete(EJBResultSet rs, Connection conn) throws SQLException {
		final EJBSQLBuilderUtils sqlBuilder = new EJBSQLBuilderUtils(_serverType);
		return execUpdate(sqlBuilder.buildDeleteSql(rs, getCallerPrincipalName()), conn);
	}
	...
	private void populateData(ResultSet rs, EJBResultSet result) throws SQLException, EJBXRuntime {
		...
		Vector<ColumnMetaData> customColumns = result.getEJBResultSetMetaData().getEditorMetaData().getCustomColumns();
		...
		Vector<EJBRecord> records = new Vector<EJBRecord>();
		Vector<EJBColumn> columns = null;
		int index = 0;
		ColumnMetaData columnMetaData = null;
		int rowIndex = 0;
		...
		while (rs.next() && ++rowIndex <= pageSize) {
			...
			columns = new Vector<EJBColumn>(count);

			// Populate the columns in the record
			for (index = 1; index <= count; index++) {

				columnMetaData = getColumnMetaData( rs.getMetaData().getColumnName(index), result.getEJBResultSetMetaData() ); 
				if (columnMetaData == null)
					throw new EJBXRuntime(_module, "ResultSet column : " + rs.getMetaData().getColumnName(index) + " does not have any MetaData configured: ");
	...
	private ColumnMetaData getColumnMetaData(String columnName, EJBResultSetMetaData resultSetMetaData) 
	{
		for (int i=0; i<resultSetMetaData.getColumnCount(); i++)
			if (resultSetMetaData.getColumnName(i).equals( columnName ))
				return resultSetMetaData.getColumnMetaData( i );
		return null;
	}

	/**
	 * Build an EJBColumn from the JDBC resultset (using the column-type etc. in
	 * the column metadata)
	 *
	 * @param rs
	...
	private EJBColumn createColumn(ResultSet rs, ColumnMetaData columnMetaData, int index) throws SQLException, EJBXRuntime {
		...
		if (columnMetaData.getType() == ColumnDataType._CDT_CLOB) {
			...
			longString = rs.getString(index);
			columnNull = longString == null;
		}
		...
		switch (columnMetaData.getType()) {
			...
			case ColumnDataType._CDT_BOOLEAN:
				column.setBoolValue(rs.getBoolean(index));
				break;
			...
			case ColumnDataType._CDT_TIMESTAMP: // System.out.println( "Date
				// with no calendar : " +
				// rs.getTimestamp( index ) );
				// System.out.println( "Date with calendar : " +
				// rs.getTimestamp( index, AVConstants._GMT_CALENDAR ) );
				// java.text.SimpleDateFormat s = new
				// java.text.SimpleDateFormat( "MM/dd/yyyy hh:mm" );
				// s.setTimeZone( AVConstants._GMT_TIMEZONE );
				// System.out.println( "Formatted Date with calendar : " +
				// s.format( rs.getTimestamp( index, AVConstants._GMT_CALENDAR )
				// ) );
	        ...
	        case ColumnDataType._CDT_BLOB     	:
	            column.setStrValue( DBUtl.blobToString(blob, column.getName()) );
	            break;
