...
public class DateUtl {
	...
	public static Date getTimestamp() {
		Date date = new Date();
		long time = date.getTime() + AVConstants._LOCAL_TIMEZONE_OFFSET
				+ (AVConstants._LOCAL_TIMEZONE.inDaylightTime(date) ? AVConstants._LOCAL_DSTSAVINGS : 0);
	...
	public static DateUtl getInstance(String cutoffYearValue) {
		if (!StrUtl.isEmptyTrimmed(cutoffYearValue)) {
			...
			if (cutoffYear == 0) {
				Date dt = new Date();
				CnfgFileMgr paramsFile = new com.addval.utils.CnfgFileMgr(_COMMON_CONFIG);
				int defaultStartYear = Integer
						.parseInt(paramsFile.getPropertyValue(_TWO_DIGIT_DEFAULT_START_YEAR, "1900"));
				int defaultNbrOfYear = Integer
						.parseInt(paramsFile.getPropertyValue(_TWO_DIGIT_DEFAULT_NBR_OF_YEAR, "79"));
				cutoffYear = (defaultStartYear + dt.getYear()) - defaultNbrOfYear;
			}
			Calendar calendar = Calendar.getInstance(AVConstants._GMT_TIMEZONE);
			// calendar is set for 0 for all except year to make sure that the
			// setting
			// take effect for all days from start of cutoffYear.
			// for eg. if the year is 2000 and If the day has 1 in the below
			// statement, then
			// 1-1-00 would be interpreted by java as 1-1-2100. - jeyaraj -
			// 06-Feb-2005
	...
	public static boolean isDateInFormat(String datePattern, String value) {
		...
		if (datePattern.equalsIgnoreCase("HH:MM")) {
			...
			// dateRegExp = "^([0-9]|[0-1][0-9]|[2][0-3])([0-5][0-9])$";
			dateRegExp = "^([0-1][0-9]{1}|[2][0-3]{1})([0-5][0-9]){1}$"; // 0000
																			// -
																			// 2359
		...
		if (!StrUtl.isEmptyTrimmed(dateRegExp)) {
			RE matcher;
			try {
				matcher = new RE(dateRegExp, RE.MATCH_CASEINDEPENDENT);
