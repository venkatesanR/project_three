...
import com.techmania.common.util.collectionutil.MultiHashKey;
import com.techmania.common.util.misc.MathUtl;
...
public abstract class AOkudTreeNode implements Comparable, Serializable {
    ...
    public int compareTo(AOkudTreeNode other) {
        return MathUtl.sign(other.getScore() - this.getScore());
    }
    ...
    protected List<AOkudTreeNode> setParentNode(List<AOkudTreeNode> valueNodes) {
        ...
        if (ListUtl.isEmpty(valueNodes) || valueNodes.get(0).getParent() != null) {
            return valueNodes;
        }

        for (AOkudTreeNode node : valueNodes) {
            node.setParent(this);
        }
